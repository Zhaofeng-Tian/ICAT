...
class IcatEnv(gym.Env):
    ...
    def _get_obs(self):
        ...
        for car in self.cars:
            ...
            points = car._get_lidar_points()
            g = car._get_goal_obs(use_vector=True)
            # obs = np.concatenate((obs,g))
            lidar_obs_list.append(obs)
            lidar_points_list.append(points)
            goal_list.append(g)
            state_list.append(car.state)
        obs = {
            "lidar": np.array(lidar_obs_list),
            "points": np.array(lidar_points_list),
            "goal": np.array(goal_list),
            "center": np.array(self.obj_centers),
            "velocity": np.array(self.obj_velocity),
            "radius": np.array(self.obj_radius),
            "state":np.array(state_list),
        }
        # print("points: ", lidar_points_list)
        return obs

    def render_frame(self, infos):
            plt.cla()
            # if self.sensor_update_type != "obj":
            self.ax.imshow(self.img, origin = 'lower',extent=[0,self.world_x,0,self.world_y])
            ...
            if self.plot_lidar:  
                for car in self.cars:
                    for end in car.points:
                        ...
                        x = [start[0], end[0]]
                        y = [start[1], end[1]]
                        self.ax.plot(x,y,color='blue',alpha = 0.2)
            ...
            plot_start_goal(self.ax, self.initial_states, self.global_goals)
            plot_cars(self.ax, self.cars)
            # plot_geo_disks(self.ax, self.obj_centers, self.obj_radius)
            # if self.plot_VO:
            #     if self.policy == "VO":
            #         plot_VOs(self.ax, infos)
            #     elif self.policy == "ORCA":
            #         plot_ORCA(self.ax, infos)
            if self.plot_VO:
                plot_ORCA(self.ax, infos)
